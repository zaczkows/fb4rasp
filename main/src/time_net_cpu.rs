...
use engine::{params::Layout, ring_buffer::FixedRingBuffer};
use fb4rasp_shared::{CpuUsage, MemInfo, NetworkInfo, RenderState, SystemInfo};
...
use crate::timeouts::NET_REFRESH_TIMEOUT;

pub(crate) fn render_time_cpu_net<DB>(state: &RenderState, mut fb: DB)
where
    for<'a> DB: Display<'a>,
{
    ...
    let layout = Layout::Vertical; // engine_handle.get_main_layout().await;
    let touch_status = Vec::<adafruit_mpr121::Mpr121TouchStatus>::new(); //engine_handle.touch_info().await;
    let (prev, last): (NetworkInfo, NetworkInfo) = (
        NetworkInfo {
            tx_bytes: 0,
            rx_bytes: 0,
        },
    ...
    ); //engine_handle.last_net_info().await;
    let sys_infos: HashMap<String, FixedRingBuffer<SystemInfo>> = HashMap::new(); // engine_handle.get_system_infos().await;
    let (tx_data, rx_data): (Vec<i64>, Vec<i64>) = (
        state.net_tx.iter().map(|v| *v).collect(),
        state.net_rx.iter().map(|v| *v).collect(),
    );
    // end
...

    {
        ...
        fb.render_text(
            ...
            &format!(
                "Bytes tx: {}, tx/s: {}",
                size::Size::Bytes(last.tx_bytes).to_string(size::Base::Base2, size::Style::Smart),
        ...
        fb.render_text(
            ...
            &format!(
                "Bytes rx: {}, rx/s: {}",
                size::Size::Bytes(last.rx_bytes).to_string(size::Base::Base2, size::Style::Smart),
...

    {
...

        {
            if !tx_data.is_empty() && !rx_data.is_empty() {
                ...
                crate::helpers::plot_data(&plot, &YELLOW, &mut color_index, left_axis, right_axis);
            }
        }
    }
